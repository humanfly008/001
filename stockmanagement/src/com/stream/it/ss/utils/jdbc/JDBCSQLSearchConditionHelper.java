package com.stream.it.ss.utils.jdbc;
...
public class JDBCSQLSearchConditionHelper {
	...
	public static String getSQLSearchCondition(SearchConditionValuesBean[]conditionValues){
		...
		if (conditionValues != null) {
			for(int i=0; i<conditionValues.length && conditionValues[i]!=null; i++){ 
				...
				if(conditionValues[i].getValues()!=null && conditionValues[i].getValues().length>0){
					...
					if(conditionValues[i].getValues()[0]!=null && !conditionValues[i].getValues()[0].equals("")){						
    ...
    public static String getSQLOrderBy(SearchBean searchBean)throws Exception{ 
        StringBuilder result = new StringBuilder();
        if (searchBean.getOrderBy()!=null && !searchBean.getOrderBy().equals("")){   
            result.append("ORDER BY ");
            result.append(searchBean.getOrderBy());
            result.append(" ");
            result.append(searchBean.getOrderType());
            result.append(" ");
            result.append("\r\n");
        }
    ...
    public static String getSQLGroupBy(SearchBean searchBean)throws Exception{ 
        StringBuilder result = new StringBuilder();
        if (searchBean.getGroupBy()!=null && !searchBean.getGroupBy().equals("")){   
            result.append("GROUP BY ");
            result.append(searchBean.getGroupBy());
            result.append(" ");
            result.append("\r\n");
        }
